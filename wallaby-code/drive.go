...
func (d DifferentialDrive) RotateDeg(deg float32) <-chan interface{} {
	...
	lSpd, rSpd := EqualSpeed(d.LeftWheel.Motor, d.RightWheel.Motor, 300 + int(700*math.Min(float64(deg/180), 1))
...
func (d DifferentialDrive) TurnDeg(deg, middleRadius float32) <-chan interface{} {
	...
	if deg > 0 {
		distL = outerDist
		distR = innerDist
		spdL = 300 + int(700*math.Min(float64(deg/180), 1))
		spdR = int(float32(spdL) * (innerDist / outerDist))
	} else {
		distR = outerDist
		distL = innerDist
		spdR = 300 + int(700*math.Min(float64(deg/180), 1))
